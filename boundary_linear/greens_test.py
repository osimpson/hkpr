...
def test_network(Net, eps=0.01, randfactor=1):
    ...
    allowable_err = eps*(np.linalg.norm(b1)+np.linalg.norm(xS_true))
    f.write(str(max(0, np.linalg.norm(xS_true-xS_hkpr) - allowable_err)))
...
f = open('sampling_tests_dolphins3.txt', 'w')
...
f = open('sampling_tests_lesmis3.txt', 'w')
...
f = open('sampling_tests_power3.txt', 'w')
...
f = open('sampling_tests_facebook3.txt', 'w')
